...
def checkSerial():  #the main loop is that
    ...
    if (useMqtt and Mqqt_client.connected_flag) :
        if (time.time() > mower.timeToSendMqttState):
            ...
            r=Mqqt_client.publish(topic="Mower/State",payload=myRobot.stateNames[mymower.state], qos=0, retain=False)
            r=Mqqt_client.publish(topic="Mower/Status",payload=myRobot.statusNames[mymower.status], qos=0, retain=False)
            r=Mqqt_client.publish(topic="Mower/Battery",payload=mymower.batVoltage, qos=0, retain=False)
            r=Mqqt_client.publish(topic="Mower/Temp",payload=mymower.Dht22Temp, qos=0, retain=False)
