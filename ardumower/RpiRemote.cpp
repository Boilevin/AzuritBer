...
void RpiRemote::RaspberryPISendByLane () {
  ...
  lineToSend = lineToSend + ",";
  lineToSend = lineToSend + robot->imuDriveHeading;
  lineToSend = lineToSend + ",";
  writePi(lineToSend);
}
...
void RpiRemote::RaspberryPISendStat () {
  ...
  lineToSend = lineToSend + ",";
  lineToSend = lineToSend + "0";//robot->odometryX;
  lineToSend = lineToSend + ",";
  lineToSend = lineToSend + "0";//robot->odometryY;
  lineToSend = lineToSend + ",";
  lineToSend = lineToSend + "0";//robot->prevYawCalcOdo;
  lineToSend = lineToSend + ",";
  lineToSend = lineToSend + robot->batVoltage;
  lineToSend = lineToSend + ",";
  lineToSend = lineToSend + robot->imu.ypr.yaw;
...
void RpiRemote::readWrite_var() {  //can be use to change the value of 4 variable in one sentence
  ...
  if (readOrWrite == 'w') {
    ...
    for (int i = 0; i <= 3; i++) {  // read the 4 values and check to adjust
      ...
      if (strncmp(variable_name[i], "PiNewHeading", 20) == 0) {
        robot->PiNewHeading = atoi(received_value[i]);
        Console.println(robot->PiNewHeading);
      }
      if (strncmp(variable_name[i], "PiNewHeadingDurat", 20) == 0) { 
        robot->PiNewHeadingEnd  = millis() + 1000 * atoi(received_value[i]);
        Console.print(millis());
        Console.print(" Heading change until : ");
        Console.println(robot->PiNewHeadingEnd);
      }
