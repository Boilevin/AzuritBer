...
void RemoteControl::sendMotorMenu(boolean update) {
  ...
  serialPort->print(F("|a32~Calib Speed and Ticks/Second"));  //to compute the ticks per second motor speed
  serialPort->println("}");
}
...
void RemoteControl::processMotorMenu(String pfodCmd) {
  ...
  if (pfodCmd.startsWith("a02")) {
    ...
    robot->stateEndOdometryRight = robot->odometryRight + 6 * robot->odometryTicksPerRevolution;
...
void RemoteControl::sendMowMenu(boolean update) {
  ...
  serialPort->print("/");
  serialPort->print(robot->Center_Mow_Power);
  serialPort->print("/");
...
void RemoteControl::sendPerimeterMenu(boolean update) {
  ...
  sendSlider("e04", F("Big AREA Smag Center"), robot->perimeterTriggerMinSmag, "", 1, 500, 100);
  ...
  sendSlider("e21", F("Perimeter MAG MAX VALUE"), robot->perimeterMagMaxValue, "", 1 , 3000, 1000);
...
void RemoteControl::sendRemoteMenu(boolean update) {
  if (update) serialPort->print("{:"); else serialPort->print(F("{.Raspberry`1000"));
