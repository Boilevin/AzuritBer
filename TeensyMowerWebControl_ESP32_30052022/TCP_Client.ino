...
void handlePFODclient() {
  ...
  if (PFODclientConnected) {
    if (!PFODclient.connected())  {
      ...
      Serial_ESP_to_USB.println(MSG_HEADER " PFOD-Client Disconnected");
    }
  }
  ...
  if (PFODserver.hasClient()) {
    ...
    if (!PFODclientConnected) {
      ...
      Serial_ESP_to_USB.println(MSG_HEADER " PFOD-Client Connected");
    } else {
  ...
  if (PFODclientConnected) {
    ...
    if (PFODclient.available()) {
      ...
      while (PFODclient.available()) {
        ...
        debugln(ESPtoPCB);Serial_ESP_to_USB.write(sbuf, len);
      }
    }
  }
