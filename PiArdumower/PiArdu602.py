...
from config import personDetectNewHeading
from config import personDetectNewHeadingDuration
from config import personDetectMinScore
...
def checkSerial():  #the main loop is that
    ...
    if (mymower.personDetect==True):
        ...
        send_var_message('w','PiNewHeading',''+str(mymower.VisionNewHeading)+'','PiNewHeadingDurat',''+str(personDetectNewHeadingDuration)+'','0','0','0','0','0')
    ...
    fen1.after(10,checkSerial)  # here is the main loop each 20ms
...
def visionThread(num):
    ...
    model_dir = '/home/pi/Documents/robotics-level-4/all_models'
    ...
    def main():
        ...
        while True:
            ...
            if (time.time() > mymower.personDetectAt + personDetectNewHeadingDuration):  #wait before next detection
                
                
                
                if ((myRobot.statusNames[mymower.status]=="WAIT") or (myRobot.statusNames[mymower.status]=="MANUAL")):
                    ...
                    for obj in objs:
                        ...
                        print(percent , " " , obj.id)
                   
                        if (int(obj.id) == 0 and percent > personDetectMinScore ): # 0 is a person and detected
                            ...
                            x = int(((x0+x1)/2)*width)
                            y = int(((y0+y1)/2)*height)
                            
                            consoleInsertText("person detect at " + str(x) + "  " + str(y) + '\n')

                            if (x > width/2) :
                                 mymower.VisionNewHeading = -1 * personDetectNewHeading
                            else :
                                 mymower.VisionNewHeading = personDetectNewHeading
                 
                 
                            consoleInsertText("Mower need to rotate " + str(mymower.VisionNewHeading)  + '\n')   
     
                            mymower.personDetect=True
                            mymower.personDetectAt = time.time()
                 
                            cv2.imwrite("/home/pi/Documents/PiArdumower/vision/visionperson%d.jpg" % count, cv2_im)
                            count=count+1

                            photo1=ImageTk.PhotoImage(image=Image.fromarray(cv2_im))
                            Video_canvas.create_image(0,0,image = photo1,anchor=tk.NW)
                            #cv2.imshow('Video1', cv2_im)
                        else:
                            #t1 = cv2.getTickCount()
                        ...
                        if (cv2.waitKey(1) == ord('q') or not(mymower.visonStart)):
                            break
    ...
    app.run(host='0.0.0.0', port=8000, threaded=True) # Run FLASK
    main()

def BtnTestVideo_click():
    consoleInsertText("Test 1234" + '\n')
    output = subprocess.run(["./test", "a.in", "0.001"], stdout = subprocess.PIPE,universal_newlines = True).stdout
...
FrameStreamVideo.place(x=0, y=0, width=600, height=400)

Video_canvas=tk.Canvas(FrameStreamVideo,bg='red')
Video_canvas.place(x=0, y=0, width=600, height=400)
...
BtnStreamVideoStart= tk.Button(StreamVideoPage,command = BtnStreamVideoStart_click,text="Start")
BtnStreamVideoStart.place(x=650,y=100, height=25, width=60)
BtnStreamVideoStop= tk.Button(StreamVideoPage,command = BtnStreamVideoStop_click,text="Stop")
BtnStreamVideoStop.place(x=650,y=130, height=25, width=60)

BtnTestVideo= tk.Button(StreamVideoPage,command = BtnTestVideo_click,text="Test 1234")
BtnTestVideo.place(x=650,y=160, height=25, width=60)


BtnVisionStart= tk.Button(StreamVideoPage,command = BtnVisionStart_click,text="Vision Start")
BtnVisionStart.place(x=650,y=200, height=25, width=80)
BtnVisionStop= tk.Button(StreamVideoPage,command = BtnVisionStop_click,text="Vision Stop")
BtnVisionStop.place(x=650,y=230, height=25, width=80)
