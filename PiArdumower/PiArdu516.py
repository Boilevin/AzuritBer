...
def checkSerial():  #the main loop is that
    ...
    if (mymower.visionDetect==True):
        #only stop if surface OK
        if (mymower.surfaceDetected > 16):
        ...
        consoleInsertText("Detected ID : " + str(mymower.objectDetectedID) + '\n')
        consoleInsertText("Score : " + str(mymower.visionScore) + '\n')
        consoleInsertText("Surface : " + str(int(mymower.surfaceDetected)) + '\n')
...
# Initialize frame rate calculation
    ...
    for frame1 in camera.capture_continuous(rawCapture, format="bgr",use_video_port=True):
        ...
        # All  results have been drawn on the frame, so it's time to display it.
                ...
                #multiple detection are possible in one frame
                #test only index 0 because it's the higher detection score
                if (int(classes[0][0]) == 1 and scores[0][0] > visionDetectMinScore/100 ) or (int(classes[0][0]) == 37 and scores[0][0] > visionDetectMinScore/100 ) or(int(classes[0][0]) == 62 and scores[0][0] > visionDetectMinScore/100 ): # 1 is a person and detected
