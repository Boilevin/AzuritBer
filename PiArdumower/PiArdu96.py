...
#################################### RFID MANAGEMENT ###############################################
if RfidConnectedOnPi :    
    print("---------------------------Start of RFID initialisation------------------")
    ...
    def rfidTagGained(e):
        source = e.device
        #rfid.setLEDOn(1)
        mymower.lastRfidFind=e.tag
        find_rfid_tag()
        consoleInsertText('RFID Tag Find: %s' % (mymower.lastRfidFind)+ '\n')
    

    def rfidTagLost(e):
        source = e.device
        #rfid.setLEDOn(0)
        consoleInsertText('RFID Tag Lost: %s' % (mymower.lastRfidFind)+ '\n')
    ...
    try:
        ...
        print("---------------------------End of RFID initialisation------------------")
    except PhidgetException as e:
        print("Phidget Exception %i: %s" % (e.code, e.details))
        print("?????????????  RFID NOT OK ?????????????????????")
...
def find_rfid_tag():
    ...
    if (mymower.newtagToDo=="Null"):
        consoleInsertText('RFID Tag Not into the file: %s' % (mymower.lastRfidFind)+ '\n')
    else:
        ...
        if str(response1)!="b''":  
            ...
            if response1[:1] != '$' : #it is console message because the first digit is not $
                ...
                try:
                    ...
                    print("PARSE ERROR FROM DUE --> " + str(response1))
                    
                    consoleInsertText("PARSE ERROR FROM DUE" + '\n')
...
""" ------------------- connecting the the laser nano tower ------------ """
try:
    if NanoConnectedOnPi :
...
def ButtonStartArea2_click():
    ...
    sub = subprocess.Popen("curl 10.42.0.254/area2/1", stdout=subprocess.PIPE, stderr=subprocess.PIPE,shell=True)
...
def ButtonStopArea2_click():
    ...
    sub = subprocess.Popen("curl 10.42.0.254/area2/0", stdout=subprocess.PIPE, stderr=subprocess.PIPE,shell=True)
    output, error_output = sub.communicate()   
...
tk.Label(RfidPage,text="Mower State:",font=("Arial", 10), fg='green').place(x=530,y=25, height=20, width=80)
