...
//Comment this line to calibrate IMU
...
void Robot::OdoRampCompute() { //execute only one time when a new state execution
  ...
  if (distToMoveLeft >= odometryTicksPerRevolution)  {
    ...
    if (UseAccelLeft && UseBrakeLeft) { //need 2 ramp
      OdoStartBrakeLeft = distToMoveLeft / 2; //on freine a la moiti̩ de la distance a parcourir
  ...
  if (distToMoveRight >= odometryTicksPerRevolution) { //more than 1 rev
    ...
    if (UseAccelRight && UseBrakeRight) {
      OdoStartBrakeRight = distToMoveRight / 2; //on freine a la moiti̩ de la distance a parcourir
  ...
  if ((!RaspberryPIUse) && (Console.available() > 0)) {  //do not read console if raspberry connected
    ...
    switch (ch) {
      ...
      case 't':
        ...
        break;
      case 'u':
        setNextState(STATE_FORWARD_ODO, RIGHT);
        break;
      case 'v':
        //bb
...
void Robot::readSensors() {
  ...
  if (millis() >= nextTimeMotorSense) {
    ...
    if (batVoltage > 8) {
      ...
      //bber1
      //tested: it's take 3 ms to read the 3 power value

      Center_Mow_Power = CenterMowIna226.readBusPower() ;
      Left_Mow_Power = LeftMowIna226.readBusPower() ;
      Right_Mow_Power = RightMowIna226.readBusPower() ;
      motorMowSense   = max(Center_Mow_Power, Left_Mow_Power);
      motorMowSense   = max(motorMowSense, Right_Mow_Power);
    }
    else {
...
/*
    //check if the ina226 work correctly
    StartReadAt = millis();
    Center_Mow_Power = CenterMowIna226.readBusPower() ;
    Left_Mow_Power = LeftMowIna226.readBusPower() ;
    Right_Mow_Power = RightMowIna226.readBusPower() ;
    shuntvoltagec = CenterMowIna226.readBusVoltage();
    shuntvoltagel = LeftMowIna226.readBusVoltage();
    shuntvoltager = RightMowIna226.readBusVoltage();
    EndReadAt = millis();
    ReadDuration = EndReadAt - StartReadAt;
    Console.print("         Read Duration in ms ");
    Console.println(ReadDuration);
    Console.print("MOW Power C/L/R ");
    Console.print("   ");
    Console.print(Center_Mow_Power);
    Console.print("   ");
    Console.print(Left_Mow_Power);
    Console.print("   ");
    Console.println(Right_Mow_Power);
    Console.print("MOW Voltage C/L/R ");
    Console.print("   ");
    Console.print(shuntvoltagec);
    Console.print("   ");
    Console.print(shuntvoltagel);
    Console.print("   ");
    Console.println(shuntvoltager);
